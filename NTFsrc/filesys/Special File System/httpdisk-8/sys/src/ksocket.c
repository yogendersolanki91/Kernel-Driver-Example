...
#pragma warning(suppress:28197)
        s1->streamSocket = (PSTREAM_SOCKET) ExAllocatePoolWithTag(NonPagedPool, sizeof(STREAM_SOCKET), KSOCKET_POOL_TAG);
...
int __cdecl connect(INT_PTR socket, const struct sockaddr *addr, int addrlen)
{
    ...
    if (s->type == SOCK_STREAM)
    {
        ...
        {
#pragma warning(suppress:28197)
            s->streamSocket = (PSTREAM_SOCKET) ExAllocatePoolWithTag(NonPagedPool, sizeof(STREAM_SOCKET), KSOCKET_POOL_TAG);
...
INT_PTR __cdecl socket(int af, int type, int protocol)
{
    ...
    s = (PSOCKET) ExAllocatePoolWithTag(NonPagedPool, sizeof(SOCKET), KSOCKET_POOL_TAG);
