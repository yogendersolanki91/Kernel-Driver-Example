...
NTSTATUS tdi_open_transport_address(PUNICODE_STRING devName, ULONG addr, USHORT port, BOOLEAN shared, PHANDLE addressHandle, PFILE_OBJECT *addressFileObject)
{
    ...
    eaBuffer = (PFILE_FULL_EA_INFORMATION) ExAllocatePoolWithTag(PagedPool, eaSize, KTDI_POOL_TAG);
...
NTSTATUS tdi_connect(PFILE_OBJECT connectionFileObject, ULONG addr, USHORT port)
{
    ...
    remoteInfo = (PTDI_CONNECTION_INFORMATION) ExAllocatePoolWithTag(NonPagedPool, 2 * sizeof(TDI_CONNECTION_INFORMATION) + 2 * sizeof(TA_IP_ADDRESS), KTDI_POOL_TAG);
...
NTSTATUS tdi_listen(PFILE_OBJECT connectionFileObject, PULONG addr, PUSHORT port)
{
    ...
    remoteInfo = (PTDI_CONNECTION_INFORMATION) ExAllocatePoolWithTag(NonPagedPool, 2 * sizeof(TDI_CONNECTION_INFORMATION) + sizeof(TA_IP_ADDRESS), KTDI_POOL_TAG);
...
NTSTATUS tdi_send_dgram(PFILE_OBJECT addressFileObject, ULONG addr, USHORT port, const char *buf, int len)
{
    ...
    remoteInfo = (PTDI_CONNECTION_INFORMATION) ExAllocatePoolWithTag(NonPagedPool, sizeof(TDI_CONNECTION_INFORMATION) + sizeof(TA_IP_ADDRESS), KTDI_POOL_TAG);
...
IO_COMPLETION_ROUTINE tdi_send_mdl_completion;
...
IO_COMPLETION_ROUTINE tdi_send_mdl_async_completion;
...
NTSTATUS tdi_send_stream_mdl_async(PFILE_OBJECT connectionFileObject, PMDL mdl, ULONG flags, void (*completionRoutine)(int status, void *context), void *context)
{
    ...
    ctx = ExAllocatePoolWithTag(NonPagedPool, sizeof(TDI_COMPLETION_CONTEXT), KTDI_POOL_TAG);
...
NTSTATUS tdi_query_address(PFILE_OBJECT addressFileObject, PULONG addr, PUSHORT port)
{
    ...
    localInfo = (PTRANSPORT_ADDRESS) ExAllocatePoolWithTag(NonPagedPool, sizeof(TDI_ADDRESS_INFO)*10, KTDI_POOL_TAG);
